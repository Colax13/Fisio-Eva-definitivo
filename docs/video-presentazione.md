# Video di presentazione — schema 30" / 40" / 50"

**Regola:** 2,2 parole al secondo. 30" = 66 parole · 40" = 88 · 50" = 110.

**La cosa che li fa fallire quasi sempre:** si aprono con «siamo». Nessuno resta a
guardare un video che comincia parlando di te. Si apre dal lato di chi guarda —
un motivo, una domanda, un problema — e «siamo» arriva dopo.

---

## Lo schema

| Battuta | 30" | 40" | 50" |
| --- | --- | --- | --- |
| **1. Aggancio** — dal lato di chi guarda, mai «siamo» | 14 | 17 | 18 |
| **2. Chi siamo** — una riga, niente curriculum | 10 | 13 | 18 |
| **3. Cosa c'è dentro** — concreto, elenco parlato | 21 | 28 | 26 |
| **4. Come si lavora** — la valutazione prima del trattamento | — | 16 | 25 |
| **5. Chiusura** — indirizzo, sempre | 17 | 11 | 24 |

Quello che si perde accorciando: a 30" salta *come si lavora*, e la chiusura si
prende gli orari. A 40" torna il metodo. A 50" ci sta anche il dopo.

---

## 30" — il biglietto da visita

*Quello da fissare in alto sul profilo. Nessun metodo, solo dove siamo e cosa c'è.*

| Tempo | Testo | Parole |
| --- | --- | --- |
| 0–6" | «Se abiti qui intorno e ti serve un fisioterapista, ti faccio vedere dove siamo.» | 14 |
| 6–11" | «Siamo FisioEVA, uno studio di fisioterapia e osteopatia a Casalotti.» | 10 |
| 11–20" | «Terapia manuale, osteopatia, riabilitazione dopo un intervento, percorsi per la donna e per i bambini. E le terapie strumentali, quando servono.» | 21 |
| 20–28" | «Aperti dal lunedì al venerdì dalle otto alle venti, e il sabato mattina. Via di Boccea 755.» | 17 |

**62 parole · 28 secondi**

---

## 40" — chi siamo

*Il video identitario. Risponde a una domanda invece di presentarsi.*

| Tempo | Testo | Parole |
| --- | --- | --- |
| 0–8" | «Ci chiedono spesso perché abbiamo aperto uno studio proprio qui a Casalotti, e non da un'altra parte.» | 17 |
| 8–14" | «Siamo in tre, fisioterapiste e osteopate, e lavoriamo in questo quartiere da anni.» | 13 |
| 14–26" | «Qui dentro c'è quello che di solito vai a cercare in tre posti diversi: terapia manuale, osteopatia, riabilitazione, il percorso donna, l'osteopatia neonatale, la tecar e il laser.» | 28 |
| 26–34" | «E prima di qualsiasi cosa c'è una valutazione: il percorso si costruisce su quello che troviamo.» | 16 |
| 34–39" | «Siamo in via di Boccea 755, a Casalotti. Passate a conoscerci.» | 11 |

**85 parole · 39 secondi**

> La battuta a 14" è il posizionamento: *quello che di solito vai a cercare in tre
> posti diversi*. Dice «siamo completi» senza dire «siamo gli unici», che non si
> può dire.

---

## 50" — la presentazione completa

*Quella per il sito, la bio, il primo contatto freddo.*

| Tempo | Testo | Parole |
| --- | --- | --- |
| 0–8" | «Il problema, quando cerchi un fisioterapista, è che non sai cosa ti serve finché non te lo dicono.» | 18 |
| 8–16" | «Noi siamo FisioEVA, studio di fisioterapia e osteopatia a Casalotti. Siamo in tre, e lavoriamo qui da anni.» | 18 |
| 16–28" | «Dentro trovi terapia manuale, osteopatia, riabilitazione dopo un intervento, il percorso donna — gravidanza, post-parto, pavimento pelvico — l'osteopatia per i neonati, e le terapie strumentali.» | 26 |
| 28–40" | «Ma la prima cosa non è un trattamento: è un colloquio di venti minuti in cui ascoltiamo la tua storia e guardiamo come ti muovi.» | 25 |
| 40–50" | «Il percorso nasce da lì. E quando finisce, al piano di sopra si continua in gruppo. Siamo in via di Boccea 755, a Casalotti.» | 24 |

**111 parole · 50 secondi**

> L'ultima battuta nomina StudioEVA senza mostrarlo. Se preferisci non citarlo
> finché non è pronto:
>
> «Il percorso nasce da lì, e si costruisce sul tuo. Siamo in via di Boccea 755, a
> Casalotti.» *(18 parole — restano 6 da recuperare altrove)*

---

## La regola: il messaggio generale viene prima

È già scritta nel codice del sito, nel commento alle porte d'ingresso dei servizi:

> «L'ordine non si tocca: la generalista per prima. Sopra la piega il messaggio
> resta neutro — la specializzazione femminile è affiancata, non sostitutiva.»

Vale identica in video. Un video di presentazione che si apre su gravidanza e
post-parto non aggiunge un pubblico: ne toglie uno, perché dice a metà del
quartiere che quel posto non è per loro. Lo stesso vale per una professionista:
presentarla partendo dalla sua nicchia la trasforma in «quella delle donne», e da
lì non torna più indietro.

Quindi: **le presentazioni sono generaliste.** Le specializzazioni esistono, ma
stanno nei video dedicati, che si fanno dopo e servono a un altro scopo.

---

# Singolo percorso — quello generalista · 40"

*È il percorso da cui entra quasi tutto: mal di schiena, cervicale, infortuni,
post-operatorio.*

| Tempo | Testo | Parole |
| --- | --- | --- |
| **0–9"** | «La maggior parte delle persone che entra qui non ha una diagnosi. Ha un dolore che dura da troppo.» | 19 |
| **9–16"** | «Siamo in tre, fisioterapiste e osteopate, e questo è il percorso da cui parte quasi tutto.» | 16 |
| **16–24"** | «Mal di schiena, cervicale, spalle, ginocchia. Infortuni sportivi, e il recupero dopo un intervento o una frattura.» | 17 |
| **24–32"** | «Ma la prima cosa non è un trattamento: è una valutazione, perché lo stesso dolore può avere cause diverse.» | 19 |
| **32–40"** | «Se te lo porti dietro da mesi, scrivici. Siamo in via di Boccea 755, a Casalotti.» | 16 |

**87 parole · 40"** — **da non perdere:** «non ha una diagnosi, ha un dolore che
dura da troppo».

---

# I terapisti, uno per uno · 40"

**Non è un curriculum letto ad alta voce.** I titoli stanno sul sito, e a
elencarli in video si perde chi guarda al terzo. Un solo titolo, una cosa in cui
crede, due o tre cose che fa. E non si apre con «mi chiamo»: si apre con qualcosa
che ha notato lavorando.

| Battuta | Contenuto | Parole |
| --- | --- | --- |
| 1. Aggancio | una cosa notata lavorando — generale, mai una nicchia | 20–26 |
| 2. Chi è | nome, ruolo, **un** titolo | 7–13 |
| 3. Cosa fa | i problemi comuni, non le specializzazioni | 18–20 |
| 4. Come lavora | la sua idea di fondo, in una frase | 18–20 |
| 5. Chiusura | **l'idea, non l'indirizzo** | 11–21 |

> **L'indirizzo non è una battuta di chiusura, è un'informazione.** Nelle prime
> versioni questi video finivano tutti su «siamo in via di Boccea 755»: il picco
> era la battuta 4 — l'idea — e poi il video si sgonfiava su un numero civico.
> L'indirizzo va in sovrimpressione e nella descrizione del post; la voce chiude
> sulla persona.
>
> Diverso per i video dello studio qui sopra: lì l'indirizzo *è* il contenuto, e
> resta in chiusura.

---

## Azzurra — 90 parole · 41"

| Tempo | Testo | Parole |
| --- | --- | --- |
| **0–9"** | «Quando una persona arriva qui con un dolore, la prima cosa che faccio quasi sempre è guardare da un'altra parte.» | 20 |
| **9–15"** | «Sono Azzurra, fisioterapista e osteopata: la formazione osteopatica serve esattamente a questo.» | 12 |
| **15–23"** | «Mi occupo di mal di schiena, cervicale, dolori articolari, postura, e del recupero dopo un intervento o un infortunio.» | 19 |
| **23–32"** | «Perché il punto dove senti male è quasi sempre quello che ha ceduto per ultimo. Non quello che ha cominciato.» | 20 |
| **32–41"** | «E se tratti solo quello, torna. Per questo la prima volta che ci vediamo non ti tratto: ti guardo.» | 19 |

**90 parole · 41"** — **da non perdere:** «la prima cosa che faccio è guardare da
un'altra parte» e «non ti tratto: ti guardo».

> La chiusura trasforma il principio in una promessa concreta su cosa succede la
> prima volta. È anche il modo più efficace di dire «la valutazione viene prima»
> senza usare la parola valutazione.

---

## Elisa — 86 parole · 39"

| Tempo | Testo | Parole |
| --- | --- | --- |
| **0–11"** | «Vent'anni fa ho scritto una tesi sul legame tra le emozioni, la postura e il respiro. E sto ancora lavorando su quella cosa lì.» | 24 |
| **11–17"** | «Sono Elisa, fisioterapista, e qui dentro mi occupo di postura e di movimento.» | 13 |
| **17–26"** | «Ginnastica posturale, terapia manuale, rieducazione del movimento. E il metodo Feldenkrais, che sto ancora studiando dopo tre anni di formazione.» | 20 |
| **26–34"** | «Perché per stare meglio, molto spesso, imparare a muoversi con meno fatica conta più che diventare più forti.» | 18 |
| **34–39"** | «E non c'è un'età in cui sia troppo tardi per impararlo.» | 11 |

**86 parole · 39"** — **da non perdere:** «sto ancora lavorando su quella cosa lì»
e «meno fatica conta più che più forti».

> La chiusura apre invece di chiudere: dice a chi ha sessant'anni e si è
> rassegnato che il video parlava anche a lui.

> «Che sto ancora studiando» va tenuto: è vero — la formazione Feldenkrais è
> quadriennale e lei è al terzo anno — e dire che è già practitioner sarebbe
> scorretto. Detto così, poi, è la cosa più credibile del video.

---

## Veronica — 91 parole · 41"

| Tempo | Testo | Parole |
| --- | --- | --- |
| **0–12"** | «Mi sono formata al CONI, a contatto con atleti di livello internazionale. E lì ho capito una cosa che vale per tutti, non solo per loro.» | 26 |
| **12–15"** | «Sono Veronica, prima osteopata e poi fisioterapista.» | 7 |
| **15–23"** | «Mi occupo di infortuni, di recupero dopo un intervento, e di rimettere in movimento chi si è fermato.» | 18 |
| **23–32"** | «La cosa è questa: non sentire più dolore non vuol dire essere pronti. Il tessuto guarisce dopo, non prima.» | 19 |
| **32–41"** | «Ed è per questo che ci si fa male di nuovo: non per sfortuna, ma perché si è rientrati troppo presto.» | 21 |

**91 parole · 41"** — **da non perdere:** «non sentire più dolore non vuol dire
essere pronti».

> La chiusura chiude il cerchio aperto al secondo zero («ho capito una cosa») e
> lascia addosso un pensiero invece di una richiesta.

> La prima battuta apre una promessa — «ho capito una cosa» — che la quarta
> mantiene. È il motivo per cui si arriva in fondo: si resta per sapere qual è.

---

## Il team clinico

Siria, Andrea, Valentina e Maria Caterina non hanno ancora un video, e non per
scelta editoriale: sul sito titolo, specializzazione e numero di albo sono ancora
da confermare, e in un video pubblico il titolo di un sanitario è un obbligo di
legge. Appena arrivano, stessa forma.

---

# Video mirati — dopo, non al posto

Quando le presentazioni generaliste sono online, questi si aggiungono. Servono a
un altro scopo: non a far conoscere lo studio, ma a farsi trovare da chi ha già
quel problema. Stesso schema, solo le battute 3 e 4 cambiano.

- **Donna** — gravidanza, post-parto, pavimento pelvico, cicatrice del cesareo
- **Bambini** — «"sono le coliche" è la frase che si usa quando non si sa»
- **Sport** — «non fa più male non vuol dire che sei pronto»
- **Terapie strumentali** — «la tecar da sola non ti sistema niente»
